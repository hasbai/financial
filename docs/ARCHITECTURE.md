...
`wrangler.jsonc` 配置 Worker financial、dist 静态目录、single-page-application 回退与 financial.hasbai.xyz 自定义域名。`public/_headers` 配置缓存、安全头。推送 main 后由 GitHub Actions `Deploy Financial` 按路径自动构建并调用 Wrangler 发布；博客由 `Deploy Blog` 独立发布。根 Wrangler 配置仍保留供本地兼容，不重复手动部署。
