...
| 视图 | 粒度与用途 |
| --- | --- |
| `home` | 当月首页的一行快照；余额、损益、待补录数、六个余额曲线点、现金配置状态、未来30天现金合计/分组和最近三笔（含分类） |
...
兼容核对用的 `Repository.overview` 组合10个并发 GET（余额合计、科目余额、损益合计、类别、日趋势、现金合计、现金类别、期初现金、累计质量和期间待补录）。这些请求分别读取数据库快照；单人保存后统一失效重查，任一查询失败整张总览报错。`quality.generated_at` 为客户端组合完成时间，不作为数据库快照时间。净资产曲线每个时点只读一次余额合计；现金流合计/分组也只查现金流视图。
...
`004_home_snapshot.sql` 增加只读 `financial.home` 视图。客户端 `GET /home?select=...` 返回当月首屏实际显示的数据；最近三笔只返回卡片字段与分类，不读取整张科目表或分录数组，不做游标探测。六个曲线点与五个现金分组在SQL内形成JSON数组；余额/损益从共享分录CTE汇总。隐藏金额初次请求不选择两个图表列，显示时再取带图表的单行快照。已有快照时，图表组件不回退到多次分片请求。

视图 `security_invoker=true`、`WHERE financial.is_owner()`，无效身份不返回快照行。仅授予SELECT，不修改原表字段、记录、ID、RLS或写函数；也不增加服务器代理把11次HTTP请求藏到后端。合并后是一条数据库语句的一致快照，内部仍要读取和聚合现有记录。

视图固定服务当月首屏，历史月份和用户主动打开的明细继续按已选范围读取原视图，不预载全部明细。发布顺序为：在隔离分支验证004迁移 → 经授权将004迁移应用生产 → 真实API验证 → 前端发布。缺少新视图时报错，不静默回退11次请求。
