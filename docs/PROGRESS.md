...
- 新增原生 Playwright：iPhone 13、iPhone SE / WebKit 优先，辅以 Android 和桌面 Chromium；25 项、34 张基线覆盖主要页面状态、隐私/深色/窄屏、键盘与触控。连续三轮 75/75 通过；故意 12px 错位使截图比较失败（21,390 像素差异），原测试随后恢复。无需 Docker，日常回归不依赖逐页人工或 AI 看图。
...
- CI新增macOS 26原生浏览器任务、覆盖率/截图/trace工件；缺少或偏离基线即失败，CI不自动更新图片。生产构建不含测试入口或fixture token。Git/CI/Cloudflare交付结果见本次最终消息。
