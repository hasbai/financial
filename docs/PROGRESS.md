...
- 私有仓库的GitHub分支保护/rulesets接口返回需要GitHub Pro，设置保护请求也被套餐限制拒绝；没有更改仓库可见性或购买套餐。目标规则为强制PR、check/visual必需且跟上main、管理员不可绕过、不可force push/删除。目前流程已约束交付，但GitHub尚未强制锁定人工合并。
